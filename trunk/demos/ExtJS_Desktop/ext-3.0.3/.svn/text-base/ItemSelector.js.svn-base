...
/**    Decide whether to drop above or below a View node. */ 
    ...
    isValidDropPoint: function(pt, n, data) { 
       ...
       /* console.log(d == n)
        console.log((pt == "below") && (n.nextSibling == d))
        console.log((pt == "above") && (n.previousSibling == d))*/
        if (d == n) { 
            return false; 
        } 
        ...
        for (var i = 0; i < data.records.length; i++) { 
            ...
            if (dup && (dd != this.dragZone)) { 
				if(!this.allowDup && !this.allowTrash){  					
					this.el.dom.scrollTop = Ext.get(this.getNode(this.store.indexOf(dup))).dom.offsetTop; 				
...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
		...
		this.upIcon = el.createChild({tag:"img",src:pltPath+"/meris/resource/image/icon/right_b2.png",style:{cursor:"pointer",margin:"2px"}})
		this.addIcon = el.createChild({tag:"img",src:pltPath+"/meris/resource/image/icon/right_b.png",style:{cursor:"pointer",margin:"2px"}})
		this.removeIcon = el.createChild({tag:"img",src:pltPath+"/meris/resource/image/icon/left_b.png",style:{cursor:"pointer",margin:"2px"}})
		this.toTopIcon = el.createChild({tag:"img",src:pltPath+"/meris/resource/image/icon/left_b2.png",style:{cursor:"pointer",margin:"2px"}})
		//el.createChild({tag: 'br'});
		if (!this.toSortField) {
		...
		if (!this.readOnly) {
			...
			this.upIcon.on('click', this.up, this);
			this.toTopIcon.on('click', this.down, this);
		}
	...
	down : function() {
		...
		var selectionsArray = [];
		var records = this.toMultiselect.view.store.getRange();
		if (records.length > 0) {			
			selectionsArray = [];
			for (var i=0; i<records.length; i++) {
				record = records[i];
