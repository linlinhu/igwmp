...
/**
 * loadding 非模态  弹层加载框  (来e杯 logo)
 */
(function($, window) {
	var CLASS_ACTIVE = 'leb-active';
	/**
	 * 自动消失提示框
	 */
	$.LEBloadding = function() {
		var toast = document.createElement('div');
		toast.classList.add('leb-loadding-message');
		toast.classList.add('leb-loading');
		toast.innerHTML = '<svg class="leb-svg-icon" id="leb-svg-loadding" width="80px" height="80px" viewBox="0 0 500 500" xml:space="preserve">' +
			'<path fill-rule="evenodd" clip-rule="evenodd" fill="#888889" d="M238.874,307.021c12.807-8.389,23.588-19.678,32.295-33.932' +
			'c8.764-14.191,13.117-26.618,13.117-37.211c0-5.298-1.263-9.335-3.849-12.11c-2.587-2.774-6.367-4.162-11.29-4.162' +
			'c-10.216,0-19.299,5.99-27.183,18.038c-7.885,11.983-11.79,25.922-11.79,41.815c0,6.055,0.689,11.291,2.014,15.645' +
			'C233.576,299.455,235.783,303.426,238.874,307.021L238.874,307.021z M231.577,89.095c7.229,0,14.457,2.757,19.975,8.271' +
			'c12.275,14.615,77.222,109.194,81.128,122.25c7.604,15.201,11.893,32.351,11.893,50.513c-0.004,62.406-50.597,112.99-112.995,112.99' +
			'c-24.111,0-46.455-7.559-64.806-20.428c5.737-0.875,11.452-2.045,17.171-3.51c11.984-3.09,23.964-7.443,35.887-13.119' +
			'c7.63,6.244,16.717,10.904,27.183,13.939c8.696,2.52,18.628,4.002,29.679,4.418c23.88,0.59,24.877-24.273,8.688-25.129' +
			'c-5.624-0.426-10.569-1.344-14.84-2.752c-6.12-2.08-11.733-5.488-16.905-10.281c17.785-13.434,31.977-28.947,42.637-46.484' +
			'c10.656-17.531,16.019-33.616,16.019-48.122c0-13.307-3.721-23.84-11.161-31.534c-7.444-7.694-17.6-11.542-30.398-11.542' +
			'c-22.898,0-42.762,9.145-59.607,27.373c-16.838,18.228-25.227,39.67-25.227,64.268c0,10.596,1.134,19.867,3.341,27.752' +
			'c2.208,7.883,5.617,14.883,10.281,21.064c-6.746,2.965-13.755,5.275-20.938,7.002c-4.296,1.029-8.832,1.883-13.587,2.514' +
			'c-1.752,0.23-10.759,1.387-21.609,2.209c-15.518-19.344-24.801-43.9-24.805-70.629c0-18.162,4.285-35.312,11.893-50.513' +
			'c3.906-13.056,68.853-107.635,81.128-122.25C217.117,91.852,224.346,89.095,231.577,89.095z" />' +
			'</svg>';

		document.body.appendChild(toast);
		toast.classList.add(CLASS_ACTIVE);
		var p = document.querySelector("#leb-svg-loadding").querySelector("path");
		setTimeout(function () {
			p.setAttribute("stroke-dashoffset","2000");
			p.setAttribute("stroke-dasharray","2000");
		},100);
		setTimeout(function() {
			toast.classList.remove(CLASS_ACTIVE);
			var div = document.getElementsByClassName("leb-loadding-message");
	...
	var Numbox = $.Numbox = $.Class.extend({
		...
		initEvent: function() {
			...
			self.minus.addEventListener(tapEventName, function(event) {
				var val;
				if (self.input.value == '1斤') {
					val = parseInt('10两') - self.options.step;
				}else {
					val = parseInt(self.input.value) - self.options.step;
				}				
				self.input.value = val.toString();
				$.trigger(self.input, changeEventName, null);
			});
		...
		checkValue: function() {
			...
			if (val == null || val == '' || isNaN(val)) {
				...
				self.input.value = val+'两';
				if (val == self.options.max) {
					self.input.value = val/10 +'斤';					
				}
				//给输入框拼接字符  两
			}
		},
