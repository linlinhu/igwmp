 ...
 * USM window
 */
Emin.WXERWindow = Ext.extend(Ext.app.Module, {
    id:'WXER-win',
    init : function(){
        this.launcher = {
            text: '菜单事件回复设置',
    ...
    createWindow : function(){
        ...
        var win = desktop.getWindow('WXER-win');
		if(!win){
				...
				items:[{
					...
					listeners:{
						select:function(c,r,i){
							if(r.data.activekey){
								replyForm.getForm().findField("eventKey").setValue(r.data.activekey);
							}else{
								Ext.getCmp("oa_menu_cbx").setRawValue("");
								showErrorMsg("此公众号菜单不可选！");
							}
						}
					}					
				},{
					...
					handler:function(){
						if(replyForm.getForm().isValid()){
							...
							replyForm.getForm().submit({
								url:"eventReply/saveOrUpdate.do",
								params:{
									event:"CLICK",
									articalIds:articalIds.join(","),
									woaId:pageOfficialAccountId,
									rContent:replyForm.getForm().findField("rcontent").getValue()
								},
								waitMsg:'正在保存...',
			...
			var replyStore = new Ext.data.JsonStore({
				url:"eventReply/list.do",
				fields:["eventKey","replyType","rContent","articles","id"]
			})
			replyStore.load()
			...
			'<div style="width:100%;text-align:center;font-size:14px">事件代码:{eventKey}</div>',
			...
			'<div style="width:300px;text-align:center;">',
          	...
          	'&nbsp;&nbsp;&nbsp;',
          	'<a href="javascript:void(0);"  name="event_remove" style="'+editDisplay+'"><img src="js/images/types/icons/cancel.png"  align="absmiddle" style="width:16px;height:16px"/>&nbsp;删除</a>',
         	'</div>',
	      	'</div>',
			'</tpl>',
			...
			var officialAccountCbx = new Ext.form.ComboBox({
					...
					listeners:{
						select:function(e){
							...
							replyStore.reload();
							menuStore.baseParams.woaId = pageOfficialAccountId;
							menuStore.reload();
						},
					...
					listeners:{
						click:function(d,i,n,e){
							...
							if(e.target.name=="event_edit"){
								...
								replyWindow.show()
								replyForm.findByType("combo")[0].setValue(record.data.eventKey)
								...
								Ext.Msg.confirm("提示","是否删除此事件回复？",function(b){
									if(b=="yes"){
										Ext.Ajax.request({
											url:"eventReply/delete.do",
											params:{
												id:record.data.id
											},
