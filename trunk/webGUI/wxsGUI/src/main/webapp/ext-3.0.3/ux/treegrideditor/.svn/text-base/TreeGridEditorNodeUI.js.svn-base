...
Ext.ux.tree.TreeGridEditorNodeUI = Ext.extend(Ext.ux.tree.TreeGridNodeUI, {
    renderElements: function(n, a, targetNode, bulkRender) {
        ...
        for (i = 1, len = cols.length; i < len; i++) {
            ...
            buf.push('<td class="x-treegrid-col ', (c.cls ? c.cls : ''), '">', '<div id="', t.id, '-node-if-', c.dataIndex, '-', n.id, '" unselectable="on" class="x-treegrid-text"', (c.align ? ' style="text-align: ' + c.align + ';"' : ''), (c.displayTip && cTip ? ' qtip="' + cTip + '"' : ''), '>', (c.tpl ? c.tpl.apply(a) : fVal), '</div>', '</td>');
        }
