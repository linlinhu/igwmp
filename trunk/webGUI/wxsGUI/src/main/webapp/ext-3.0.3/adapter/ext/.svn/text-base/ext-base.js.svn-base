	...
	Ext.apply(Ext, {
		...
		override : function(e, D) {
			if (D) {
				var C = e.prototype;
				Ext.apply(C, D);
				if (Ext.isIE && D.toString != e.toString) {
		...
		toArray : function() {
			return r ? function(e, E, C, D) {
				D = [];
				Ext.each(e, function(F) {
							D.push(F)
						});
				return D.slice(E || 0, C || D.length)
			} : function(e, D, C) {
...
(function() {
	...
	Ext.lib.Event = function() {
		var x = false, v = [], g = [], C = 0, p = [], d, E = false, k = window, I = document, l = 200, s = 20, D = 0, r = 1, i = 2, m = 3, t = 3, y = 4, u = "scrollLeft", q = "scrollTop", f = "unload", A = "mouseover", H = "mouseout", e = function() {
			var J;
			if (k.addEventListener) {
				J = function(N, L, M, K) {
					if (L == "mouseenter") {
						M = M.createInterceptor(o);
						N.addEventListener(A, M, (K))
					} else {
						if (L == "mouseleave") {
							M = M.createInterceptor(o);
							N.addEventListener(H, M, (K))
						} else {
							N.addEventListener(L, M, (K))
						}
					}
					return M
				}
			...
			if (k.removeEventListener) {
				J = function(N, L, M, K) {
					...
					N.removeEventListener(L, M, (K))
				}
			} else {
				if (k.detachEvent) {
					J = function(M, K, L) {
						M.detachEvent("on" + K, L)
					}
				} else {
					J = function() {
					}
		...
		function o(J) {
			return !w(J.currentTarget, z.getRelatedTarget(J))
		}
		function w(J, K) {
			if (J && J.firstChild) {
				while (K) {
		...
		function B(P, L, O) {
			for (var K, M = -1, J = v.length, N = J - 1; N >= 0; --N) {
				K = v[N];
				if (K && K[i] == O && K[D] == P && K[r] == L) {
					M = N;
					break
				}
			}
		...
		function F() {
			var J = false, M = [], K, L = !x || (C > 0);
			if (!E) {
				E = true;
				Ext.each(p, function(O, P, N) {
							if (O && (K = I.getElementById(O.id))) {
								if (!O.checkReady || x || K.nextSibling
										|| (I && I.body)) {
									K = O.override ? (O.override === true
											? O.obj
											: O.override) : K;
									O.fn.call(K, O.obj);
									p[P] = null
		...
		function G() {
			var J = I.documentElement, K = I.body;
			if (J && (J[q] || J[u])) {
				return [J[u], J[q]]
			} else {
				if (K) {
		...
		var z = {
			...
			removeListener : function(O, K, N) {
				...
				O = Ext.getDom(O);
				if (!N) {
					M = this.purgeElement(O, false, K)
				} else {
					if (f == K) {
						Ext.each(g, function(Q, R, P) {
									if (Q && Q[0] == O && Q[1] == K
											&& Q[2] == N) {
										g.splice(R, 1);
										M = true
			...
			getTarget : function(J) {
				J = J.browserEvent || J;
				return this.resolveTextNode(J.target || J.srcElement)
			},
			...
			getRelatedTarget : function(J) {
				J = J.browserEvent || J;
				return this.resolveTextNode(J.relatedTarget
						|| (J.type == H ? J.toElement : J.type == A
								? J.fromElement
								: null))
			},
			getPageX : function(J) {
			...
			getXY : function(J) {
				return [this.getPageX(J), this.getPageY(J)]
			},
			stopEvent : function(J) {
				this.stopPropagation(J);
				this.preventDefault(J)
			},
			...
			getEvent : function(J) {
				J = J || k.event;
				if (!J) {
					var K = this.getEvent.caller;
					while (K) {
						J = K.arguments[0];
						if (J && Event == J.constructor) {
							break
						}
						K = K.caller
					}
			...
			getCharCode : function(J) {
				J = J.browserEvent || J;
				return J.charCode || J.keyCode || 0
			},
			_load : function(K) {
				x = true;
				var J = Ext.lib.Event;
			...
			purgeElement : function(K, M, J) {
				var L = this;
				Ext.each(L.getListeners(K, J), function(N) {
							if (N) {
								L.removeListener(K, N.type, N.fn, N.index)
							}
						});
				if (M && K && K.childNodes) {
					Ext.each(K.childNodes, function(N) {
								L.purgeElement(N, M, J)
							})
				}
			},
			...
			_unload : function(Q) {
				var P = Ext.lib.Event, N, M, K, J, L, O;
				Ext.each(g, function(R) {
							if (R) {
								try {
									O = R[y]
											? (R[y] === true ? R[t] : R[y])
											: k;
									R[i].call(O, P.getEvent(Q), R[t])
								} catch (S) {
								}
							}
						});
				...
				if (v && (M = v.length)) {
					while (M) {
						if ((K = v[L = --M])) {
							P.removeListener(K[D], K[r], K[i], L)
						}
					}
				}
				h(k, f, P._unload)
