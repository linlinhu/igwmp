...
//  // 业务广播标识
    ...
    var ACTION_CABINET_NETOWRK = "cabinet.network.h5";//网络状态变化
    var ACTION_CABINET_UPGRADE = "cabinet.upgrade.h5";//升级通知
//
    var pluginCommon = {
...
//
    ...
    function initReceiver(callback){
        var _callback = typeof callback !== 'function' ? null : function(type,args) {
            callback(type,args);
        };
        ...
        receiver = plus.android.implements('io.dcloud.android.content.BroadcastReceiver', {
            onReceive: function(context, intent) { //实现onReceiver回调函数
                ...
                if(broadcast == ACTION_CABINET_NETOWRK){
                	var param = bundle.getString(PARAM_KEY);
                    _callback(0,param);
                }else if(broadcast == ACTION_CABINET_UPGRADE){
                	var param = bundle.getString(PARAM_KEY);
                	_callback(1,param);
                }
           }});
...
//  
    ...
    function createJob(callback) {
        ...
        filter.addAction(ACTION_CABINET_NETOWRK);
        filter.addAction(ACTION_CABINET_UPGRADE);
        mainActivity.registerReceiver(receiver,filter);
        // --- 注册监听 end ---

    }
//
    window.plus.com = pluginCommon;
}, true);