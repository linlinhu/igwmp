...
public class LruCache<K, V> implements Cache<K, V> {
    ...
    @Override
    public final V get(K key) throws RunException{
        if(EmptyUtils.isEmpty(key)){
            throw new RunException(ExceptionCode.T_PARAMETER_ERROR,"key == null");
        }
        synchronized (this) {
            V value = map.get(key);
            if (value != null) {
    ...
    @Override
    public final V put(K key, V value) throws RunException{
    ...
    @Override
    public final V remove(K key) throws RunException{
        if(EmptyUtils.isEmpty(key)){
            throw new RunException(ExceptionCode.T_PARAMETER_ERROR,"key == null");
        }
        V previous;
